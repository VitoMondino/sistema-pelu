...
const cajaController = {
  ...
  cerrarCaja: async (req, res) => {
    try {
      ...
      const result = await db.query("CALL CerrarCaja(?, ?, ?, ?)", [
        caja_id,
        usuario_id,
        monto_cierre,
        observaciones || null,
    ...
    try {
      ...
      if (!tipo_movimiento || !monto || !descripcion || !usuario_id) {
        return res.status(400).json({
          success: false,
          message:
            "Tipo de movimiento, monto, descripción y usuario son requeridos",
        });
      }
  ...
  generarReporte: async (req, res) => {
    try {
      const { fecha_desde, fecha_hasta, tipo_movimiento, usuario_id } =
        req.query;
      ...
      const movimientos = await db.query(
        `
                ...
                ${whereClause}
                ORDER BY vmd.fecha_movimiento DESC`,
        params
      );
  ...
  obtenerHistorial: async (req, res) => {
    try {
      ...
      const fechaCierreDesde = req.query.fecha_cierre_desde; // Nuevo parámetro
      const fechaCierreHasta = req.query.fecha_cierre_hasta; // Nuevo parámetro
      ...
      const cajas = await db.query(
        `
            ...
            LEFT JOIN usuarios u_cierre ON c.usuario_cierre_id = u_cierre.id
            LEFT JOIN movimientos_caja mc ON c.id = mc.caja_id
            ${fechaCierreDesde ? "WHERE c.fecha_cierre >= ?" : ""}
            ${fechaCierreHasta ? "AND c.fecha_cierre <= ?" : ""}
            GROUP BY c.id
            ORDER BY c.fecha_apertura DESC
            LIMIT ${limit} OFFSET ${offset}
        `,
        [fechaCierreDesde, fechaCierreHasta].filter(Boolean)
      );
      ...
      for (const mov of movimientos) {
        if (!movimientosPorCaja[mov.caja_id])
          movimientosPorCaja[mov.caja_id] = [];
