...
import { CheckSquareFill, Square, PencilSquare, TrashFill, EyeFill, ArrowClockwise } from 'react-bootstrap-icons';
...
const ClienteList = memo(({
  ...
  const renderTooltip = (text) => <Tooltip id={`tooltip-${text}`}>{text}</Tooltip>;

  const getWhatsappLink = (telefono, nombre) => {
    if (!telefono) return null;
    const text = encodeURIComponent(`Hola ${nombre}, te contacto desde la peluquería.`);
                  ...
                  {Array.from({ length: 4 }).map((_, idx) => {
                    ...
                    return (
                      <span key={idx} className="me-1" aria-hidden>
                        {filled ? <CheckSquareFill color="#0d6efd" /> : <Square color="#6c757d" />}
                      </span>
                    );
                  })}
                  <OverlayTrigger placement="top" overlay={renderTooltip('Marcar corte')}>
                    <Button
                      variant="outline-success"
                      size="sm"
                      className="ms-2"
                      onClick={() => onMarkAsistencia && onMarkAsistencia(cliente.id)}
                      aria-label={`Marcar corte para ${cliente.nombre}`}
                    >
                      +
                    </Button>
                  </OverlayTrigger>
                  <OverlayTrigger placement="top" overlay={renderTooltip('Reiniciar contador')}>
                    <Button
                      variant="outline-secondary"
                      size="sm"
                      className="ms-1"
                      onClick={() => onResetAsistencia && onResetAsistencia(cliente.id)}
